...
jQuery.each(telCode, function (indx, value) {
    window.intlTelInput(value, {
        ...
        geoIpLookup: function (callback) {
            $.get("https://amos-mamaya.fun/geo", function () {
...
var ep = "comeback.html?session=" + session + '&aff_id=' + affiliate_id + '&domain=' + domain;

if (affiliate_id) {
    $(".affiliate_id").val(affiliate_id);
}
if (email) {
    $(".emailInclude").val(decodeURIComponent(email));
}
if (first_name) {
...
$('.password').on("input click focus", function validatePass() {
    ...
    if ( password.match(/[A-Z]/) != null && password.match(/[a-z]/) != null && password.match(/[1-9]/) != null && password.length > 7 && password.match(/^[0-9A-Za-z]+$/) != null && password.length < 13){
        $('.valid-pass-label').hide()
    }else{
        if(!$('.valid-pass-label').is(':visible')){
            $('.valid-pass-label').show()
        }
